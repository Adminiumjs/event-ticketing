/**
 * The same-origin session transport's TIMEZONE contract, against canned wire
 * responses.
 *
 * THE FIXTURES BELOW ARE COPIED FROM A LIVE SERVER, not written from the route
 * handlers. An earlier version in this fleet inferred them and got three of
 * four envelopes wrong — `connections` is not under `data`, and `csrfToken` is
 * under `data` — and every test still passed, because the port and the fixture
 * shared one wrong assumption. Change a fixture only against a real response.
 *
 * These assert the CONTRACT with `apps/server`, so both URLs and envelope
 * shapes below are copied from the server rather than invented:
 * `/api/v1/bootstrap` (csrfToken), `/api/v1/connections` and
 * `/api/v1/connections/:id/schema`.
 */
import { describe, expect, it, vi } from "vitest";

import { SessionPortError, sessionPort } from "./sessionSource.ts";
import { TABLE_OF_REF } from "./tableOfRef.ts";

function harness(opts: { conns?: unknown[]; schema?: Record<string, unknown> } = {}) {
  const fetchImpl = vi.fn(async (input: unknown) => {
    ...
    if (path === "/api/v1/bootstrap") payload = { data: { csrfToken: "csrf-abc" } };
      ...
      payload = {
        connections: opts.conns ?? [
          { id: "conn-1", name: "Venue DB", timezone: "Europe/Lisbon", currency: "EUR" },
        ],
      };
    else if (opts.schema !== undefined && path in opts.schema) payload = opts.schema[path];
...
/* `orders` is mapped in TABLE_OF_REF but ABSENT from SCHEMA_OK on purpose: the
...
const SCHEMA_OK = {
  "/api/v1/connections/conn-1/schema": {
    model: {
      tables: [
        { name: "venues", columns: [{ name: "id" }, { name: "name" }] },
        { name: "events", columns: [{ name: "id" }, { name: "venue_id" }] },
      ],
    },
  },
};
...
describe("discovery", () => {
  ...
  it("says the connection is PAUSED rather than missing when it is", async () => {
     ...
     */
    const { fetchImpl } = harness({ conns: [{ id: "a", name: "Venue DB", disabled: true }] });
    const port = sessionPort({ tableOfRef: TABLE_OF_REF, fetchImpl });
    await expect(port.config()).rejects.toMatchObject({ code: "CONNECTION_PAUSED" });
    await expect(port.config()).rejects.toThrow(/Venue DB/);
  });
...
describe("assertRefs", () => {
  it("points at a PAUSED connection when the schema does not match", async () => {
    ...
    const { fetchImpl } = harness({
      ...
      conns: [
        ...
        { id: "conn-2", name: "c_event_ticketing", disabled: true },
      ],
    });
    const port = sessionPort({ tableOfRef: TABLE_OF_REF, fetchImpl });
    const err = await port
      .assertRefs({ orders: ["id"] })
    ...
    expect(message).toContain('table "orders" is absent');
    expect(message).toContain("c_event_ticketing is paused in Adminium");
  ...
  it("adds no paused hint when nothing is paused", async () => {
    ...
    const { fetchImpl } = harness({ schema: SCHEMA_OK });
    const port = sessionPort({ tableOfRef: TABLE_OF_REF, fetchImpl });
    const err = await port
      .assertRefs({ orders: ["id"] })
      .catch((e: unknown) => e as SessionPortError);
