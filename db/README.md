...
`docker compose up` gives you a working product: `events-db` creates the
...
By default these talk to the `events-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/waveform npm run demo:status
```
